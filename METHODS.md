...
- **Decision:** Provide a terminal viewer as a read-only diagnostic over one
  ...
  \(N\)-by-\(N\) board with one terminal column per cell and one blank separator
  column between neighboring cells, the controls, or the required colors is an
  explicit startup failure. The actual-history glyphs are `#` for live and `.`
  for dead. Board column \(j\) is drawn at terminal column \(1+2j\) inside the
  border, so adjacent live cells remain diagnostically distinct. Retrodiction
  colors likewise occupy one terminal column per cell, with `#` as the optional
  actual-live overlay; separator columns remain uncolored. The top and bottom
  borders remain contiguous. Terminal character dimensions are controlled by
  the terminal emulator and are not stretched by the viewer.
- **Evidence:** The console viewer was requested for bidirectional trajectory
  ...
  duplicate closing state. A one-character-per-cell display with blank
  separators was subsequently selected because duplicated glyphs obscured
  diagnostic positions while adjacent one-column glyphs could visually merge.
